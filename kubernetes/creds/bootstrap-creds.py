...
def main():
  ...
  roles_certs = []
  for role in roles:
    role_cert = create_cert(ca_crt, ca_key, role['common_name'], role['subjectAltNames'])
...
def init_ca(ca_info):
  ...
  builder = builder.subject_name(x509.Name([
    ...
    x509.NameAttribute(NameOID.ORGANIZATION_NAME, ca_info['common_name'].split(' ')[0])
  ]))
  ...
  logger.info('Write %s certificate and key in %s %s', ca_info['common_name'], ca_crt_filename, ca_key_filename)
...
def create_cert(ca_crt, ca_key, common_name, subjectAltNames):
  logger.info('Create certificate for %s with sans %s', common_name, ','.join(subjectAltNames))
  one_day = datetime.timedelta(1, 0, 0)
  issuer_name = ca_crt.issuer.get_attributes_for_oid(x509.name.NameOID.COMMON_NAME)[0].value
  private_key = rsa.generate_private_key(
    public_exponent = 65537,
    key_size = 2048,
    backend = default_backend()
  ...
  logger.info('Write %s certificate and key in %s %s', common_name, ca_crt_filename, ca_key_filename)
